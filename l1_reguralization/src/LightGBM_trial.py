...
#%% load data
use_data = pd.read_csv('../input/winequality-red.csv',sep=";")

#%% make validation(simple train-test split)
X = use_data.drop('quality', axis=1)
y = use_data['quality']
...
feature_importances = np.sort(RF_model.feature_importances_)[::-1]
...
#%%
drop_features = ['fixed acidity', 'volatile acidity']
#%% lightgbm datasets
...
#%% 
lgb.plot_importance(model_l1,title='L1 importance')

#%%
lgb.plot_importance(model_fs,title='Feature selection importances')
#%% 
